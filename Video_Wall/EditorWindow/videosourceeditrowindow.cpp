#include "videosourceeditrowindow.h"
...
#include "../GraphicsView/graphicswidget.h"


VideoSourceEditroWindow::VideoSourceEditroWindow(QWidget *parent)
	: QDialog(parent)
{
#if PIX_1080p
...
VideoSourceEditroWindow::~VideoSourceEditroWindow()
{
}

void	VideoSourceEditroWindow::InitConnect()
{
	connect(m_ptrCloseBtn.get(), SIGNAL(clicked()), this, SLOT(OnButtonClose()));
}
 
void VideoSourceEditroWindow::InitUI()
{ 
  	m_ptrGraphicsWidget.reset(new GraphicsWidget(this));

	InitButtons();
	InitLabels();
}
 
void VideoSourceEditroWindow::InitButtons()
{
...
void VideoSourceEditroWindow::InitLabels()
 {
	...
	m_ptrTitle->setText(QString::fromLocal8Bit("视频编辑器"));
	m_ptrTitle->setNormalBgColor(QColor(50, 50, 50));
	m_ptrTitle->setNormalTextColor(QColor(220, 220, 220));
}
 
void VideoSourceEditroWindow::OnButtonClose()
{
	close();
 }
 
void VideoSourceEditroWindow::resizeEvent(QResizeEvent *pEvent)
 {
	...
	if (m_ptrGraphicsWidget)
	{
		m_ptrGraphicsWidget->setGeometry(0, 0,809, 650);
		m_ptrGraphicsWidget->move(9, 31);
	}

	QWidget::resizeEvent(pEvent);
  }

void VideoSourceEditroWindow::paintEvent(QPaintEvent* pEvent)
 {
...
void VideoSourceEditroWindow::mousePressEvent(QMouseEvent * pEvent)
 {
	//只能是鼠标左键移动和改变大小
...
void VideoSourceEditroWindow::mouseReleaseEvent(QMouseEvent*pEvent)
{
	m_bMousePress = false;
 }
 
void VideoSourceEditroWindow::mouseMoveEvent(QMouseEvent *pEvent)
{
	//移动窗口
	if (m_bMousePress)
	{
