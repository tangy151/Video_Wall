...
void	PlayListEditorWindow::InitConnect()
{
	connect(m_ptrCloseBtn.get(), SIGNAL(clicked()), this, SLOT(OnButtonClose()));

}

void PlayListEditorWindow::InitUI()
{
	InitButtons();
	InitLabels();
}

void PlayListEditorWindow::InitButtons()
 {
...
void PlayListEditorWindow::InitLabels()
{
	...
	m_ptrTitle->setText(QString::fromLocal8Bit("播出列表编辑器"));
	m_ptrTitle->setNormalBgColor(QColor(50, 50, 50));
	m_ptrTitle->setNormalTextColor(QColor(220, 220, 220));
}


void PlayListEditorWindow::OnButtonClose()
{
	close();
 }
 
void PlayListEditorWindow::resizeEvent(QResizeEvent *pEvent)
 {
...
void PlayListEditorWindow::paintEvent(QPaintEvent* pEvent)
{
...
void PlayListEditorWindow::mousePressEvent(QMouseEvent * pEvent)
 {
	//只能是鼠标左键移动和改变大小
...
void PlayListEditorWindow::mouseReleaseEvent(QMouseEvent*pEvent)
{
	m_bMousePress = false;
 }

void PlayListEditorWindow::mouseMoveEvent(QMouseEvent *pEvent)
{
	//移动窗口
	if (m_bMousePress)
	{
