#include "localstreameditorwindow.h"
...
LocalStreamEditorWindow::LocalStreamEditorWindow(QWidget *parent)
	: QDialog(parent)
{
#if PIX_1080p
...
LocalStreamEditorWindow::~LocalStreamEditorWindow()
{
}
void	LocalStreamEditorWindow::InitConnect()
{
	connect(m_ptrCloseBtn.get(), SIGNAL(clicked()), this, SLOT(OnButtonClose()));
 }

void LocalStreamEditorWindow::InitUI()
{
	InitButtons();
	InitLabels();
}

void LocalStreamEditorWindow::InitButtons()
...
void LocalStreamEditorWindow::InitLabels()
{
	...
	m_ptrTitle->setText(QString::fromLocal8Bit("实时源编辑器"));
	m_ptrTitle->setNormalBgColor(QColor(50, 50, 50));
	m_ptrTitle->setNormalTextColor(QColor(220, 220, 220));
}

void LocalStreamEditorWindow::OnButtonClose()
{
	close();
 }
 
void LocalStreamEditorWindow::resizeEvent(QResizeEvent *pEvent)
 {
...
void LocalStreamEditorWindow::paintEvent(QPaintEvent* pEvent)
{
...
void LocalStreamEditorWindow::mousePressEvent(QMouseEvent * pEvent)
 {
	//只能是鼠标左键移动和改变大小
...
void LocalStreamEditorWindow::mouseReleaseEvent(QMouseEvent*pEvent)
{
	m_bMousePress = false;
 }

void LocalStreamEditorWindow::mouseMoveEvent(QMouseEvent *pEvent)
{
	//移动窗口
	if (m_bMousePress)
	{
