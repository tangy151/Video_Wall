#include "ipstreameditorwindow.h"
...
IPStreamEditorWindow::IPStreamEditorWindow(QWidget *parent)
	: QDialog(parent)
{
#if PIX_1080p
...
IPStreamEditorWindow::~IPStreamEditorWindow()
{
}
void	IPStreamEditorWindow::InitConnect()
{
	connect(m_ptrCloseBtn.get(), SIGNAL(clicked()), this, SLOT(OnButtonClose()));
 }

void IPStreamEditorWindow::InitUI()
{
	InitButtons();
	InitLabels();
}

void IPStreamEditorWindow::InitButtons()
{
...
void IPStreamEditorWindow::InitLabels()
{
	...
	m_ptrTitle->setText(QString::fromLocal8Bit("网络流编辑器"));
	m_ptrTitle->setNormalBgColor(QColor(50, 50, 50));
	m_ptrTitle->setNormalTextColor(QColor(220, 220, 220));
}
 
void IPStreamEditorWindow::OnButtonClose()
{
	close();
}
 
void IPStreamEditorWindow::resizeEvent(QResizeEvent *pEvent)
 {
...
void IPStreamEditorWindow::paintEvent(QPaintEvent* pEvent)
{
...
void IPStreamEditorWindow::mousePressEvent(QMouseEvent * pEvent)
 {
	//只能是鼠标左键移动和改变大小
...
void IPStreamEditorWindow::mouseReleaseEvent(QMouseEvent*pEvent)
{
	m_bMousePress = false;
 }

void IPStreamEditorWindow::mouseMoveEvent(QMouseEvent *pEvent)
{
	//移动窗口
	if (m_bMousePress)
	{
